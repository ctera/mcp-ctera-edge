...
async def ctera_edge_filer_who_am_i(ctx: Context) -> str:
    """
    Get the current user on CTERA Edge Filer.
    """
...
async def ctera_edge_filer_list_dir(path: str, ctx: Context) -> list[dict]:
    """
    List a directory on CTERA Edge Filer.
    """
    edge = ctx.request_context.lifespan_context.session
    return [{
        'name': f.name,
...
async def ctera_edge_filer_create_directory(path: str, ctx: Context) -> str:
    """
    Create a directory on CTERA Edge Filer.
    """
    edge = ctx.request_context.lifespan_context.session
...
async def ctera_edge_filer_makedirs(path: str, ctx: Context) -> str:
    """
    Create multiple directories on CTERA Edge Filer.
    """
    edge = ctx.request_context.lifespan_context.session
...
async def ctera_edge_filer_copy_item(source: str, destination: str, ctx: Context) -> str:
    """
    Copy a file or directory on CTERA Edge Filer.
    """
    edge = ctx.request_context.lifespan_context.session
...
async def ctera_edge_filer_move_item(source: str, destination: str, ctx: Context) -> str:
    """
    Move a file or directory on CTERA Edge Filer.
    """
    edge = ctx.request_context.lifespan_context.session
...
async def ctera_edge_filer_upload_item(
    ...
    """
    Upload a file to CTERA Edge Filer.
    """
    edge = ctx.request_context.lifespan_context.session
...
@mcp.tool()
@with_session_refresh
async def ctera_edge_filer_download_file(path: str, destination: str, ctx: Context) -> str:
    """
    Download a file from CTERA Edge Filer.
    """
    edge = ctx.request_context.lifespan_context.session
...
async def ctera_edge_filer_read_file(path: str, ctx: Context) -> str:
    """
    Read a file from CTERA Edge Filer.
    """
    edge = ctx.request_context.lifespan_context.session
